...
-- Processes
  ...
  Min_Critical_Delay : constant Duration := 0.01;
  Max_Critical_Delay : constant Duration := 0.05;

  Exit_Protocol_Delay : constant Duration := 0.001;

  subtype Index is Integer range 0 .. Nr_Of_Processes - 1;
  Flag : array(Index) of Boolean := (others => False);
  turn : Index := 0;
...
-- Types, procedures and functions
  ...
  task body Process_Task_Type is
   ...
   -- ENTRY PROTOCOL
   procedure Peterson_Entry is
      Other : constant Index := 1 - Process.Id;
   begin
      Flag(Process.Id) := True;
      turn := Other;   -- give priority to the other
   ...
   -- EXIT PROTOCOL
   procedure Peterson_Exit is
   begin
         Flag(Process.Id) := False;
         delay Exit_Protocol_Delay;
   end Peterson_Exit;
